...
void	*ft_thread_calc(void *arg)
{
	...
	while (coord.x < 150)
	{
		while (coord.y < 600)
		{	
			ft_color_pick(win_data->color,
				(*win_data->f)(win_data->var));
			ft_put_pixel_img(win_data, coord.x, coord.y, win_data->color);
			win_data->var.param.im += win_data->step;
			coord.y += 1;

		}
		win_data->var.param.re += win_data->step;
...
void	ft_frac_multithread(t_win_data *win_data)
{
	...
	while (i < 4)
	{
		...
		win_data->var.param.re = win_data->re_min + win_data->step
			* i * 150;
		win_data->var.param.im = win_data->im_min;
		data[i] = *win_data;
...
int ft_julia_f(t_var var)
{
	...
	i = 0;
	while (ft_cplx_norme(var.param) < 2 && i < 30)
	{
		i += 1;
...
int	ft_mandel(t_var var)
{
	...
	i = 0;
	while (ft_cplx_norme(var.mandel_z) < 2 && i < 99)
	{
